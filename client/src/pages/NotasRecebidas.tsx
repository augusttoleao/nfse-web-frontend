...
export default function NotasRecebidas() {
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-foreground mb-2">Notas Recebidas</h1>
        <p className="text-muted-foreground">Consulte as notas fiscais recebidas por sua empresa</p>
      </div>
          ...
          <table className="w-full">
            <thead className="bg-secondary border-b border-border">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-sm font-semibold text-foreground">Emitente (CNPJ)</th>
                ...
                notasFiltradas.map((nota) => (
                  <tr key={nota.numero} className="border-b border-border hover:bg-secondary transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-foreground">
                      {nota.cnpj ? `${nota.cnpj.slice(0, 2)}.${nota.cnpj.slice(2, 5)}.${nota.cnpj.slice(5, 8)}/${nota.cnpj.slice(8, 12)}-${nota.cnpj.slice(12)}` : '-'}
                    </td>
                    <td className="px-6 py-4 text-sm text-foreground">
